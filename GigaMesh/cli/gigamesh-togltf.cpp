...
bool convertMeshData(
    ...
    if (rWriteNormals) {
           //!if the mesh which is read doesn't include normals --> recompute
           //! otherwise the gltf export is faulty
           if(std::isnan(someMesh.getVertexByIdx(0)->getNormalX())){
...
void printHelp( const char* rExecName ) {
    ...
    std::cout << "GigaMesh Software Framework TO.GLTF 3D-data" << std::endl << std::endl;
    std::cout << "Converts the given meshes to Graphic Language Transmission Format (GLTFs)." << std::endl;
...
//! Main routine for loading a (binary) PLY and store it as GLTF
//==============================================================================================================================================================
int main( int argc, char *argv[] ) {
	...
	while( ( character = getopt_long_only( argc, argv, ":s:bnkvh",
		...
		switch(character) {
			...
			case 'v':
                std::cout << "GigaMesh Software Framework TO.GLTF Graphic Language Transmission Format Files (GLTFs) " << VERSION_PACKAGE << endl;
