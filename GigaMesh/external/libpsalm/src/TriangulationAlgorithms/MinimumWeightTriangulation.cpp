...
bool MinimumWeightTriangulation::apply_to(mesh& input_mesh)
{
				...
				// with j == 3
	{
		for(size_t i = 0; i < n-j; i++)
		{
			...
			// Find minimum
			ktuple min_weight = ktuple(std::numeric_limits<double>::max(), std::numeric_limits<double>::max());
			...
			for(size_t m = i+1; m < k; m++)
			{
				...
				double area	= weights[i][m].second+weights[m][k].second+cur_weight.second;
				double angle	= std::max(	cur_weight.first,
								std::max(	weights[i][m].first,
										weights[m][k].first));

				ktuple res = ktuple(angle, area);
				if(res < min_weight)
				{
					min_weight = res;
					min_index = m;
...
bool MinimumWeightTriangulation::construct_triangulation(mesh& input_mesh, size_t i, size_t k)
{
	...
	if(i+2 == k)
	{
		if(input_mesh.add_face(	input_mesh.get_vertex(i),
					input_mesh.get_vertex(i+1),
					input_mesh.get_vertex(k)) == nullptr)
		{
