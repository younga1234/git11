...
typedef std::pair<double, double> ktuple;
/*!
...
inline ktuple MinimumWeightTriangulation::minimum_area(const vertex* v1, const vertex* v2, const vertex* v3)
{
	if(v1 == NULL || v2 == NULL || v3 == NULL)
		return(ktuple(std::numeric_limits<double>::max(), 0.0));	// ensure that invalid data does not
...
{
	if(v1 == NULL || v2 == NULL || v3 == NULL)
		return(ktuple(	std::numeric_limits<double>::max(),
