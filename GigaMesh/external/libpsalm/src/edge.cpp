...
void edge::set_g(face* g)
{
	...
	if(	f != nullptr && this->g != nullptr &&
		g != nullptr) // warning is not shown if the second face is _reset_
	{
		if(!warning_shown)
		{
...
{
	if(boundary == -1)
		boundary = (f == nullptr) || (g == nullptr) ? 1 : 0;
