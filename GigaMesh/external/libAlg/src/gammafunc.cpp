...
double gamma(double x) noexcept
{
    ...
    z = 1;
    while(ap::fp_greater_eq(x,3))
    {
        x = x-1;
        z = z*x;
    }
    while(ap::fp_less(x,0))
    {
        if( ap::fp_greater(x,-0.000000001) )
        {
    ...
    while(ap::fp_less(x,2))
    {
        if( ap::fp_less(x,0.000000001) )
        {
            result = z/((1+0.5772156649015329*x)*x);
            return result;
        }
...
double lngamma(double x, double& sgngam)
{
    ...
    return result;
#else
    return ialglib::_i_lngamma(x, sgngam);
#endif
}
