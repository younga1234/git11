     ...
     ap::real_1d_array& wgauss)
{
    ...
    int l;
    int k;
    ap::real_1d_array xgtmp;
    ap::real_1d_array wgtmp;
    ...
    for(i = 0; i <= ap::iceil(double(3*(n/2))/double(2)); i++)
    {
    ...
    //
    ta.setlength(ap::ifloor(double(3*n)/double(2))+1);
    ap::vmove(&ta(0), 1, &alpha(0), 1, ap::vlen(0,ap::ifloor(double(3*n)/double(2))));
    alpha.setlength(2*n+1);
    ap::vmove(&alpha(0), 1, &ta(0), 1, ap::vlen(0,ap::ifloor(double(3*n)/double(2))));
    for(i = ap::ifloor(double(3*n)/double(2))+1; i <= 2*n; i++)
    {
        alpha(i) = 0;
    }
    ta.setlength(ap::iceil(double(3*n)/double(2))+1);
    ap::vmove(&ta(0), 1, &beta(0), 1, ap::vlen(0,ap::iceil(double(3*n)/double(2))));
    beta.setlength(2*n+1);
    ap::vmove(&beta(0), 1, &ta(0), 1, ap::vlen(0,ap::iceil(double(3*n)/double(2))));
    for(i = ap::iceil(double(3*n)/double(2))+1; i <= 2*n; i++)
    {
        beta(i) = 0;
    }
    ...
    for(m = 0; m <= n-2; m++)
    {
        ...
        ap::vmove(&ta(0), 1, &t(0), 1, ap::vlen(0,wlen-1));
        ap::vmove(&t(0), 1, &s(0), 1, ap::vlen(0,wlen-1));
        ap::vmove(&s(0), 1, &ta(0), 1, ap::vlen(0,wlen-1));
    }
    for(j = n/2; j >= 0; j--)
    {
     ...
     ap::real_1d_array& wgauss)
{
    ...
    if( ap::fp_greater(ap::machineepsilon,1.0E-32)&&(n==15||n==21||n==31||n==41||n==51||n==61) )
    {
        info = 1;
     ...
     ap::real_1d_array& wgauss)
{
    ...
    clen = ap::iceil(double(3*(n/2))/double(2))+1;
    a.setlength(clen);
    b.setlength(clen);
    for(i = 0; i <= clen-1; i++)
    {
     ...
     ap::real_1d_array& wgauss)
{
    ...
    mu0 = 2;
    alen = ap::ifloor(double(3*(n/2))/double(2))+1;
    blen = ap::iceil(double(3*(n/2))/double(2))+1;
    alpha.setlength(alen);
    beta.setlength(blen);
    for(k = 0; k <= alen-1; k++)
    {
        alpha(k) = 0;
     ...
     double& eps)
{
    ...
    ap::ap_error::make_assertion(n==15||n==21||n==31||n==41||n==51||n==61, "GKQNodesTbl: incorrect N!");
    x.setbounds(0, n-1);
    wkronrod.setbounds(0, n-1);
    wgauss.setbounds(0, n-1);
    for(i = 0; i <= n-1; i++)
    {
        x(i) = 0;
        wkronrod(i) = 0;
    ...
    eps = ap::maxreal(ap::machineepsilon, 1.0E-32);
    if( n==15 )
    {
        ng = 4;
