...
void MeshGLShader::glPaint() {
	...
	// TODO: Implement proper parameter passing from MeshWidget to MeshGL
	// Currently disabled due to architectural mismatch (MeshGLShader doesn't have access to MeshWidget)
	// Need to add DONGARCH_CLIP_* parameters to MeshGLParams and propagate values through MeshWidget
	/*
	bool dongArchClipPreviewEnabled = false;
	getParamFlagMeshWidget( MeshWidgetParams::DONGARCH_CLIP_PREVIEW_ENABLED, &dongArchClipPreviewEnabled );
	if( dongArchClipPreviewEnabled ) {
		// Get clip plane parameters
		double clipPlaneX = 0.0, clipPlaneY = 0.0, clipPlaneZ = 1.0, clipPlaneW = 0.0;
		getParamFloatMeshWidget( MeshWidgetParams::DONGARCH_CLIP_PLANE_X, &clipPlaneX );
		getParamFloatMeshWidget( MeshWidgetParams::DONGARCH_CLIP_PLANE_Y, &clipPlaneY );
		getParamFloatMeshWidget( MeshWidgetParams::DONGARCH_CLIP_PLANE_Z, &clipPlaneZ );
		getParamFloatMeshWidget( MeshWidgetParams::DONGARCH_CLIP_PLANE_W, &clipPlaneW );

		// Set up OpenGL clipping plane (GL_CLIP_PLANE1)
		GLdouble clipPlaneEquation[4] = { clipPlaneX, clipPlaneY, clipPlaneZ, clipPlaneW };
		glClipPlane( GL_CLIP_PLANE1, clipPlaneEquation );
		glEnable( GL_CLIP_PLANE1 );
		PRINT_OPENGL_ERROR( "glEnable( GL_CLIP_PLANE1 ) - DongArch Clip Preview" );
		//cout << "[MeshGLShader::" << __FUNCTION__ << "] GL_CLIP_PLANE1 ENable (DongArch Clip)." << endl;
	} else {
		glDisable( GL_CLIP_PLANE1 );
		PRINT_OPENGL_ERROR( "glDisable( GL_CLIP_PLANE1 )" );
		//cout << "[MeshGLShader::" << __FUNCTION__ << "] GL_CLIP_PLANE1 DISable." << endl;
	}
	*/
