...
layout(std430, binding = 0) buffer FragmentCounter
{
...
void main(void)
{
    ...
    //uint fragListPos = atomicCounterIncrement(fragmentCounter);



    //careful of multisampling. Then fragments have to be stored by sample!
    vec4 fragment = shadeFragment();
    ...
    if(fragment.a != 0.0)
    {
	uint fragListPos = atomicAdd(fragmentCounter, 1);
        //premultiply alpha

	if(int(fragListPos) >= uMaxFragments)    //check if the fragment could be stored in the list
	{
	    discard;
	    return;
	}
        ...
        //no spinlock version
        fragData[fragListPos].pNext = atomicExchange(fragList[listID].pHead, int(fragListPos)); //set the pnext to head, and fraglistpos to head
	fragData[fragListPos].Color = packUnorm4x8(fragment);
        fragData[fragListPos].depth = gl_FragCoord.z;
        atomicAdd(fragList[listID].numberOfFragments,1);
    }
