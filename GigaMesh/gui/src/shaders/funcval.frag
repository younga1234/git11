...
// DongArch v4 Phase 4: Clip plane for real-time mesh cutting
uniform bool uDongArchClipEnabled = false;
uniform vec4 uDongArchClipPlane = vec4(0.0, 0.0, 1.0, 0.0); // (nx, ny, nz, d)
...
// --- MAIN ----------------------------------------------------------------------------------------------------------------------------------------------------
void main(void) {
	// DongArch v4 Phase 4: Clip plane test
	// Discard fragments on the negative side of the clip plane
	if (uDongArchClipEnabled) {
		float dist = dot(gVertex.ec_pos.xyz, uDongArchClipPlane.xyz) + uDongArchClipPlane.w;
		if (dist < 0.0) {
			discard;
		}
	}
